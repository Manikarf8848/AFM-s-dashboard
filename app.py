...
if _all_parts or uploaded_files:
    ...
    with tab["📋 AFM General"]:
        ...
        # ── Non-blocking exclusions (plus Amnesty & DLC which get their own columns) ──
        ...
        if not optional_cols["Blocking"]:
            ...
            # ── Normalise Blocking values (handles Yes/No, True/False, 1/0) ────
            def _norm_bl(val):
                if pd.isna(val):
                    return "No"
            ...
            # Work on a fresh copy so we don't pollute fdf for other tabs
            _base = fdf.copy()
            ...
            with gf3:
                gen_andon_filter = st.multiselect(
                    ...
                    help="Narrow table to selected andon types only."
                )
            ...
            if _gdf.empty:
                ...
                # ── DEBUG: show unique Blocking values to confirm normalisation ─
                unique_bl = _gdf["_BL"].value_counts().to_dict()
                orig_vals = _gdf["Blocking"].astype(str).unique().tolist()[:8]
                st.caption(
                    f"Blocking column sample values: {orig_vals} → "
                    f"Normalised: {unique_bl}"
                )

                all_res = sorted(_gdf["Resolver"].unique())

                # ── Segment data ───────────────────────────────────────────────
                # 1. ANY andon where station is blocked
                bl_df  = _gdf[_gdf["_BL"] == "Yes"].copy()
                # 2. Amnesty regardless of blocking flag
                am_df  = _gdf[_gdf["Andon Type"] == "Amnesty"].copy()
                # 3. Drive Lacking Capability regardless of blocking flag
                dlc_df = _gdf[_gdf["Andon Type"] == "Drive Lacking Capability"].copy()
                # 4. Non-blocking (BL=No), excluding the excluded types AND Amnesty/DLC
                nb_df  = _gdf[
                    (_gdf["_BL"] == "No") &
                ...
                grd = pd.DataFrame({
                    ("Blocking",           "Count"):    [c_bl_g],
                    ("Blocking",           "Avg (min)"): [a_bl_g],
                ...
                # ── Hours Lost KPIs (right below table) ───────────────────────
                st.markdown("<br>", unsafe_allow_html=True)
                ...
                t_bl   = _gdf[(_gdf["Date"] == today_date) & (_gdf["_BL"] == "Yes")]
                all_bl = _gdf[_gdf["_BL"] == "Yes"]
                ...
                def _build_gen_excel(tbl, gfdf):
                        ...
                        for (cat, sub), cn in col_map.items():
                            ...
                            if is_grand:
                                ...
                                if cat == "Blocking":
                                    ...
                                    svs = [float(tbl.loc[i, (cat, sub)]) for i in data_rows_idx if not pd.isna(tbl.loc[i, (cat, sub)])]
                                    if len(svs) >= 2:
                                        mn, mx = min(svs), max(svs)
                                        if mx != mn:
                                            norm = (val - mn) / (mx - mn)
                    ...
                    for i, hdr in enumerate(["Date", "Blocking Andon Type", "Count", "Minutes Lost", "Hours Lost"], 1):
                        ...
                        cell.fill = HDR; cell.font = HFNT; cell.border = BDR; cell.alignment = CTR
                    daily_bl2 = (
                        gfdf[gfdf["_BL"] == "Yes"]
                        .groupby(["Date", "Andon Type"])
                    ...
                    daily_bl2["Hours"] = (daily_bl2["Minutes"] / 60).round(3)
                    daily_bl2["Minutes"] = daily_bl2["Minutes"].round(2)
                    daily_bl2 = daily_bl2.sort_values(["Date", "Andon Type"])
                    for ri, row in daily_bl2.iterrows():
                        ws2.cell(row=ri+2, column=1, value=str(row["Date"]))
                        ws2.cell(row=ri+2, column=2, value=row["Andon Type"])
                        ws2.cell(row=ri+2, column=3, value=int(row["Count"]))
                        ws2.cell(row=ri+2, column=4, value=row["Minutes"])
                        ws2.cell(row=ri+2, column=5, value=row["Hours"])
