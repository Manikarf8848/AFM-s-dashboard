        ...
        if not optional_cols.get("Blocking", False):
            ...
            with gf1:
                gen_hidden_resolvers = st.multiselect("Hide Resolvers", 
                    options=sorted(fdf["Resolver"].dropna().unique()), default=[], key="gen_hide_resolvers")
            with gf2:
                gen_show_resolvers = st.multiselect("Show Only These Resolvers", 
                    options=sorted(fdf["Resolver"].dropna().unique()), default=[], key="gen_show_resolvers")
