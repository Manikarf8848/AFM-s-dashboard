...
if _all_parts or uploaded_files:
    ...
    with tab["📋 AFM General"]:
        ...
        # Andon types that are excluded from Non-Blocking column entirely
        ...
        if not optional_cols["Blocking"]:
            ...
            # ── Normalise Blocking: handles yes/no, Yes/No, TRUE/FALSE, 1/0 ──
            def _norm_bl(v):
                if pd.isna(v):
                    return "no"
                return "yes" if str(v).strip().lower() in ("yes","true","1","y") else "no"

            # Work on a fresh copy — never mutate fdf
            _base = fdf.copy()
            _base["_BL"] = _base["Blocking"].apply(_norm_bl)
            # Fill blank/NaN resolvers with "System"
            ...
            with gf2:
                gen_show = st.multiselect(
                    ...
                    help="If anything is selected, ONLY these resolvers are shown."
                )
            with gf3:
            ...
            if _gdf.empty:
                ...
                # ── Segment data ───────────────────────────────────────────────
                # Amnesty: andon type = Amnesty (blocking or not)
                am_df  = _gdf[_gdf["Andon Type"] == "Amnesty"].copy()
                # Drive Lacking Capability: andon type = DLC (blocking or not)
                dlc_df = _gdf[_gdf["Andon Type"] == "Drive Lacking Capability"].copy()
                # Blocking: Blocking=yes, excluding Amnesty & DLC (they have own columns)
                bl_df  = _gdf[
                    (_gdf["_BL"] == "yes") &
                ...
                # Build rows — column order: Amnesty | DLC | Blocking | Non-Blocking | Total
                rows = {}
                for res in all_res:
                ...
                # ── Colour legend ──────────────────────────────────────────────
                st.markdown(f"""
                <div style="display:flex;gap:12px;flex-wrap:wrap;margin-top:6px;margin-bottom:4px;font-size:0.78rem;">
                    <span style="background:rgb(210,40,40);color:white;padding:2px 10px;border-radius:10px;font-weight:700;">🔴 Blocking ≥ {THRESH_BLOCKING} min</span>
                    <span style="background:rgb(210,40,40);color:white;padding:2px 10px;border-radius:10px;font-weight:700;">🔴 Amnesty ≥ {THRESH_AMNESTY} min</span>
                    <span style="background:rgb(210,40,40);color:white;padding:2px 10px;border-radius:10px;font-weight:700;">🔴 DLC ≥ {THRESH_DLC} min</span>
                    <span style="background:rgb(210,40,40);color:white;padding:2px 10px;border-radius:10px;font-weight:700;">🔴 Non-Blocking ≥ {THRESH_NON_BLOCKING} min</span>
                ...
                t_bl   = _gdf[(_gdf["Date"] == today_date) & (_gdf["_BL"] == "yes") & (~_gdf["Andon Type"].isin(["Amnesty","Drive Lacking Capability"]))]
                all_bl = bl_df.copy()
                t_min  = t_bl["Resolve_Min"].sum()
                t_hrs  = t_min / 60
                t_cnt  = len(t_bl)
                tot_hrs = all_bl["Resolve_Min"].sum() / 60
                ...
                for box, lbl, val, sub, color in [
                    ...
                    (hl4, "Total Blocking Andons",  f"{len(all_bl):,}",     "in filtered range",                    "#3949ab"),
                ]:
                    box.markdown(f"""
                ...
                def _build_gen_excel(tbl, gfdf):
                    ...
                    # Cat thresholds for excel colouring
                    _exc_thresh = {
                        "Amnesty":            THRESH_AMNESTY,
                        "Drive Lacking Cap.": THRESH_DLC,
                        "Blocking Andons":    THRESH_BLOCKING,
                    ...
                    # Row 1: resolver label + merged category headers
                    ws.cell(row=1, column=1, value="Login").fill = HDR
                    ...
                    for r_idx, resolver in enumerate(tbl.index, 3):
                        ...
                        cell.border = BDR
                        cell.fill = GRFL if is_grand else _PF2("solid", fgColor="FFFFFF")
                        cell.font = GRFN if is_grand else NFT
                ...
                with dl_g2:
                    gen_pdf = make_tab_pdf("AFM General", _gdf, within_threshold)
                    if gen_pdf:
                        st.download_button(
                        ...
                        st.caption("Install reportlab or fpdf2 to enable PDF export.")
